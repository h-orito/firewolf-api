...
class SpectateSayDomainService : SayTypeDomainService {
    override fun isViewable(
        ...
        player: Player?,
        day: Int,
    ): Boolean = village.isViewableSpectateSay(player) || day == 0 || myself?.isViewableSpectateSay() ?: false

    override fun isSayable(
        village: Village,
        myself: VillageParticipant?,
        player: Player?,
    ): Boolean = village.isSayableSpectateSay() && myself?.isSayableSpectateSay() ?: false
}
