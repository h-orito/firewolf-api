...
class SympathizeSayDomainService : SayTypeDomainService {
    override fun isViewable(
        ...
        player: Player?,
        day: Int,
    ): Boolean = village.isViewableSympathizeSay(player) || myself?.isViewableSympathizeSay() ?: false

    override fun isSayable(
        village: Village,
        myself: VillageParticipant?,
        player: Player?,
    ): Boolean = village.isSayableSympathizeSay() && myself?.isSayableSympathizeSay() ?: false
}
