...
class MonologueSayDomainService : SayTypeDomainService {
    override fun isViewable(
        ...
        player: Player?,
        day: Int,
    ): Boolean = village.isViewableMonologueSay(player) || myself?.isViewableMonologueSay() ?: false

    override fun isSayable(
        village: Village,
        myself: VillageParticipant?,
        player: Player?,
    ): Boolean = village.isSayableMonologueSay() && myself?.isSayableMonologueSay() ?: false
}
